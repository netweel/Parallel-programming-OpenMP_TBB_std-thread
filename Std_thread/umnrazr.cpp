...
#include "../../../modules/task_4/kiseleva_razr_umn_std/umnrazr.h"
...
ComplexMatr umn_parallel_std(ComplexMatr valA, ComplexMatr valB,
    std::vector<int> rowsA, std::vector<int> rowsB, std::vector<int> indexA,
    std::vector<int> indexB, int na, int ma, int nb, int mb) {
    ComplexMatr C(na * mb);
    if (ma == nb) {
        ...
        na = ma;
        ma = tmp;
        int thr_count = static_cast<int>(std::thread::hardware_concurrency());
        ...
        auto thread_partial_calculation =
            ...
            int delta = ma / thr_count;
            for (int thr_id = 0; thr_id < thr_count - 1; thr_id++) {
                int beg_l = thr_id * delta;
                thr_vec.emplace_back(std::thread(
                    thread_partial_calculation, beg_l, beg_l + delta));
            }
            thr_vec.emplace_back(std::thread(
                thread_partial_calculation, (thr_count - 1) * delta, ma));

            for (std::thread& thread : thr_vec) {
                thread.join();
            }
    } else {
        throw SIZE_ERROR;
